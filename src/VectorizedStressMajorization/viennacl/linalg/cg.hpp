...
namespace detail
{
  ...
  */
  ...
  template<typename MatrixT, typename VectorT, typename PreconditionerT, bool thread = false>
  void solve_impl_2D(MatrixT const & matrix,
	  VectorT  & rhs,
	  cg_tag const & tag,
	  PreconditionerT const & precond,
	  bool(*monitor)(VectorT const &, typename viennacl::result_of::cpu_value_type<typename viennacl::result_of::value_type<VectorT>::type>::type, void*) = NULL,
	  void *monitor_data = NULL)
  {
	  ...
	  if (g_res[thread] == 0 || reinterpret_cast<VectorT *>(g_res[thread])->size() != rhs.size())
	  {
		  g_res[thread] = new VectorT(rhs.size());
		  g_p[thread] = new VectorT(rhs.size());
		  g_tmp[thread] = new VectorT(rhs.size());
		  //std::cout << "renew " << GetCurrentThreadId() <<"\n"<< std::endl;
	  }
	  VectorT &residual = *reinterpret_cast<VectorT *>(g_res[thread]);
	  VectorT &p = *reinterpret_cast<VectorT *>(g_p[thread]);
	  p.stream = rhs.stream;
	  residual.stream = rhs.stream;
	  residual = rhs;
	  p = rhs;
	  VectorT &tmp = *reinterpret_cast<VectorT *>(g_tmp[thread]);// = VectorT(rhs.size());
	  tmp.stream = rhs.stream;
	  ...
	  for (unsigned int i = 0; ip_rr>1e-8; ++i)
	  {
		  tmp = viennacl::linalg::prod(matrix, p);
...
struct Params {
	...
	VectorT init_guess_;
	Params(MatrixT const &A, VectorT &b,   cg_tag const &tag_) 
		: A(A), b(b), tag_(tag_)
	{}
};
cudaStream_t nonblocking;
template<typename MatrixT, typename VectorT, bool thread = false>
THREAD_RETURN WINAPI concurrent_solver(THREAD_PARAM param) {
	Params<MatrixT, VectorT>* p = (Params<MatrixT, VectorT> *)param;
	//auto err = cudaGetLastError();
	//if (err != cudaSuccess)
	//{
	//	std::cout << "Err" << cudaGetErrorName(err) << std::endl;
	//	exit(23);
	//}
	if (p->init_guess_.size() > 0) // take initial guess into account
	{
		p->b = p->b - viennacl::linalg::prod(p->A, p->init_guess_);
		detail::solve_impl_2D<MatrixT, VectorT, viennacl::linalg::no_precond, thread>(p->A, p->b, p->tag_, viennacl::linalg::no_precond(), NULL, NULL);
		p->b +=p->init_guess_;
	}
	else
		detail::solve_impl_2D<MatrixT, VectorT, viennacl::linalg::no_precond, thread>(p->A, p->b, p->tag_, viennacl::linalg::no_precond(), NULL, NULL);

	if (p->init_guess_.size()!=p->b.size()) {
		p->init_guess_.resize(p->b.size());
	}
	cudaMemcpy(p->init_guess_.handle().cuda_handle().get(), p->b.handle().cuda_handle().get(), p->b.size()*sizeof(float), cudaMemcpyDeviceToDevice);
	return 0;
}
...
public:
  ...
  cg_solver(cg_tag const & tag) : tag_(tag), monitor_callback_(NULL), user_data_(NULL) {
	  Param1[0] = Param2[0] = Param1[1] = Param2[1]= 0;
  }
  ...
  template<typename MatrixT, bool swap = false>
  void operator()(MatrixT const & A, VectorT  & b, VectorT &b2) //no return
  {
	  ...
	  if (Param1[swap] == 0)
	  {
		  Param1[swap] = new Params<MatrixT, VectorT>(A, b, tag_);
		  Param2[swap] = new Params<MatrixT, VectorT>(A, b2, tag_);
	...
	//	  cudaStreamCreateWithFlags(&nonblocking, cudaStreamNonBlocking);
		  //b.set_stream(stream1);
		  //b2.set_stream(stream2);
	  }
#ifdef _WIN32 
	  h1 = CreateThread(0, 0,
		  ...
		  0, 0);
	 // concurrent_solver<MatrixT, VectorT, true>(0);
	  h2 = CreateThread(0, 0, concurrent_solver<MatrixT, VectorT, true>, (LPVOID)(Param2[swap]), 0, 0);

	  WaitForSingleObject(h1, INFINITE);
	  WaitForSingleObject(h2, INFINITE);

#elif __unix__
	  printf("pthread round 1");

	  pthread_create(&h1, 0, concurrent_solver<MatrixT, VectorT>, Param1[swap]);
	  pthread_create(&h2, 0, concurrent_solver<MatrixT, VectorT,true>, Param2[swap]);

	  pthread_join(h1, NULL);
	  pthread_join(h2, NULL);

#else
	  printf("Fatal Error: Unsupported operating system.\n");
	  exit(24);
#endif
