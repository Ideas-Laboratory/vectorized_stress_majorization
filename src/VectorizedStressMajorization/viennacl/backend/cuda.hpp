...
{
  void * dev_ptr = NULL;
  //std::cout <<"viennacl::mem_create"<< cudaGetErrorName(cudaGetLastError()) << std::endl;
  ...
  if (!host_ptr)
  {
	  handle_type handle = handle_type(reinterpret_cast<char *>(dev_ptr), detail::cuda_deleter<char>());
	  return handle;
  }
